...
describe("SingleLevelDropdown tests", () => {
  ...
  // Empty means "no caller-supplied level", so localStorage/default still apply below.
  const level = "";
  ...
  test("a caller-supplied level wins over localStorage", async () => {
    localStorage.setItem("sld-explicit", "L");
    ...
    render(
      <SingleLevelDropdown
        ...
        level={"G"}
        setLevel={setLevel}
        controlId="sld-explicit"
      />,
    );

    expect(await screen.findByDisplayValue("Graduate")).toBeInTheDocument();
  });
});
